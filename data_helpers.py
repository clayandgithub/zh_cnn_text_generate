...
def sentence_start_padding(sentence, window_size, start_padding = '<START_PADDING>'):
    sentence_len = len(sentence)
    if sentence_len <= window_size:
        padding_sentence = [start_padding] * (window_size - sentence_len)
        padding_sentence.extend(sentence)
    else:
        padding_sentence = sentence[:window_size]
    return padding_sentence 

def load_data_file(input_text_file, window_size, start_padding = '<START_PADDING>', end_padding = '<END_PADDING>'):
    all_sentences = read_and_clean_zh_file(input_text_file)
    all_padding_sentences = []
    for sentence in all_sentences:
        if len(sentence) > 0:
            words = [start_padding] * window_size
            words.extend(sentence.split(' '))
            words.append(end_padding)
            for idx in range(len(words) - window_size):
                all_padding_sentences.append(words[idx:idx + window_size + 1]) 
    return all_padding_sentences
