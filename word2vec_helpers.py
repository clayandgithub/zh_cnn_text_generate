...
def sample_similar_word_by_vector(w2vModel, input_vec, topn = 10, temperature = 1.0):
    similar_words = w2vModel.similar_by_vector(input_vec, topn = topn)
    similarities = np.array([word[1] for word in similar_words])
    preds = np.log(similarities) / temperature
    exp_preds = np.exp(preds)
    preds = exp_preds / np.sum(exp_preds)
    probas = np.random.multinomial(1, preds, 1)
    return similar_words[np.argmax(probas)][0]

def load_model(filepath):
    w2vModel = Word2Vec.load(filepath)
    return w2vModel
